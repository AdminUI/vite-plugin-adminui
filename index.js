...
const resolveServer = (env) => {
    ...
    if (isHttps && isLinux) {
        ...
        try {
            ...
            serverConfig.host = domain;
            serverConfig.https = {
                cert: `${valetDir}/${domain}.crt`,
                key: `${valetDir}/${domain}.key`
            }
        } catch (err) {
...
export default function vitePluginAdminui({ version = "3", mode = "production" } = {}) {
    ...
    return [
        ...
        {
            name: 'adminui-basic-ssl',
            applyToEnvironment(environment) {
                if (environment.config.mode === "development" && environment.config.env.VITE_SERVER_HTTPS === "basic") {
                    return [viteBasicSslPlugin];
                }
               else {
                   return [];
               }
